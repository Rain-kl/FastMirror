...
from core.hybrid_handler import HybridHandler
...
hybrid_handler: HybridHandler = None


@asynccontextmanager
async def lifespan(app: FastAPI):
    """应用生命周期管理"""
    global cache_manager, proxy_handler, local_handler, hybrid_handler
    ...
    if app_config.mode == RunMode.PROXY:
    ...
    elif app_config.mode == RunMode.HYBRID:
        if not app_config.target_url:
            logger.error("半代理模式需要指定 target_url")
            sys.exit(1)
        ...
        logger.info(
            f"启动半代理模式: {app_config.target_url} -> http://{app_config.host}:{app_config.port}"
        )
    ...
    if hybrid_handler:
        await hybrid_handler.close()


# 创建 FastAPI 应用
app = FastAPI(
    title="FastMirror",
    description="反代框架 - 支持反代模式、本地模式和半代理模式",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def catch_all(request: Request, path: str = ""):
    ...
    if app_config.mode == RunMode.PROXY:
        ...
        return await hybrid_handler.handle_request(request, path)
    return None
...
def main():
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["proxy", "local", "hybrid"],
        help="运行模式: proxy(反代模式), local(本地模式), hybrid(半代理模式) [默认: 从 .env 读取或 proxy]",
    )
    parser.add_argument(
    ...
    # 验证反代模式和半代理模式必须提供 target_url
    if app_config.mode in [RunMode.PROXY, RunMode.HYBRID] and not app_config.target_url:
        logger.error(f"{app_config.mode.value}模式必须指定目标服务器!")
